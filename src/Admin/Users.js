...
function Users() {
  ...
  const getDatas=()=>{
    axios.get(`http://localhost/learner-api/users.php`).then(function(response) {
  ...
  const handleSubmit = async(e) => {
    ...
    try{
      let url='';
      if(inputs.id!=''){
        url='http://localhost/learner-api/users_update.php';
      }else{
  ...
  const deleteUser = (id) => {
    axios.delete(`http://localhost/learner-api/users_delete.php?id=${id}`).then(function(response){
      getDatas();
    });
  }
